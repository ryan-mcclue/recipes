from .common import *

recipe = [
  Ingredient("chicken-breast", 1000, INGREDIENT_CAT.MEAT),
  Ingredient("greek-yoghurt", 1, INGREDIENT_CAT.DAIRY),
  Ingredient("garlic", 4, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("ginger", 1, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("garam-masala", 1.5, INGREDIENT_CAT.CONDIMENT),
  Ingredient("chilli-powder", 1.5, INGREDIENT_CAT.CONDIMENT),
  Ingredient("coriander", 2, INGREDIENT_CAT.CONDIMENT),
  Ingredient("cumin", 2, INGREDIENT_CAT.CONDIMENT),
  Ingredient("smoked-paprika", 1, INGREDIENT_CAT.CONDIMENT),
  Ingredient("tumeric", 1, INGREDIENT_CAT.CONDIMENT),
  Ingredient("lemon", 1, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("egg", 1, INGREDIENT_CAT.OTHER),
  Ingredient("white-vinegar", 1, INGREDIENT_CAT.OTHER),
  Ingredient("dijon-mustard", 1, INGREDIENT_CAT.OTHER),
  Ingredient("peanut-oil", 180, INGREDIENT_CAT.OTHER),
  Ingredient("fresh-dill", 15, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("fresh-parsley", 10, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("turkish-bread", 1, INGREDIENT_CAT.OTHER),
  Ingredient("avocado", 2, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("red-onion", 1, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("lettuce", 1, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("cucumber", 1, INGREDIENT_CAT.FRUIT_VEG),
]

'''
1. Mix tandoori seasoning in bowl.
   Mix in butterflied chicken breast.
   Marinate for at least 30 minutes.
2. IMPORTANT: use jar from immersion blender and on high setting to ensure emulsification
   Add egg, vinegar, salt, mustard and herbs.
   Add oil last.
   Let settle for a minute.
   Keep immersion blender firmly on bottom for 10-15seconds until mayonnaise reaches top.
   Then move blender up and down to combine.
3. Sear chicken for 3-4minutes per side.
   Rest to let juices redistribute.
3. Cut turkish bread in half and oil the sides.
   Broil at 180celsius for about 2 minutes.

Tandoori Chicken - 

1kg (2.2lbs) - Chicken Thigh, Boneless & Skinless  
1 Cup (250g) - Natural Greek Yogurt 
4 - Garlic Cloves, Grated
20g - Ginger, Peeled & Grated
1 1/2 Tbsp (10g) - Garam Masala 
1 1/2 Tbsp (10g) - Kashmiri Chilli powder 
2 tsp (6g) - Ground Coriander 
2 tsp (6g) - Ground Cumin 
1 Tbsp (5g) - Smoked or Regular Paprika 
1 tsp (3g) - Turmeric 
1/4 Lemon, Juiced 
2 Tbsp (40ml) - Vegetable Oil 
Salt & Pepper 

Herby Mayonnaise - 

1 - Whole Egg 
1 1/2 tsp (15ml) - White Vinegar 
1 tsp (10g) - Dijon Mustard 
180ml (200g) - Neutral Oil
15g (0.5oz) - Fresh Dill 
10g (0.3oz) - Fresh Flat Leaf Parsley 
Seasoning To Taste 
'''
