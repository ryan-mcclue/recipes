from .common import *

recipe = [
  Ingredient("chicken-breast", 700, INGREDIENT_CAT.MEAT),
  Ingredient("egg", 2, INGREDIENT_CAT.OTHER),
  Ingredient("onion-powder", 1, INGREDIENT_CAT.CONDIMENT),
  Ingredient("garlic-powder", 1, INGREDIENT_CAT.CONDIMENT),
  Ingredient("plain-flour", 4, INGREDIENT_CAT.OTHER),
  Ingredient("panko-breadcrumbs", 150, INGREDIENT_CAT.OTHER),
  Ingredient("brown-onion", 2, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("carrot", 2, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("garlic", 3, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("ginger", 1, INGREDIENT_CAT.FRUIT_VEG),
  Ingredient("curry-powder", 2, INGREDIENT_CAT.CONDIMENT),
  Ingredient("cayenne-pepper", 1, INGREDIENT_CAT.CONDIMENT),
  Ingredient("soy-sauce", 1, INGREDIENT_CAT.OTHER),
  Ingredient("rice-vinegar", 1, INGREDIENT_CAT.OTHER),
  Ingredient("chicken-stock", 1, INGREDIENT_CAT.OTHER),
  Ingredient("honey", 1, INGREDIENT_CAT.OTHER),
  Ingredient("white-rice", 200, INGREDIENT_CAT.OTHER),
]

'''
1. Juliene onions. Cut carrots at angle of similar width.
   Fry and soften.
2. Slice garlic. Dice ginger. 
   Add in with salt for 5minutes.
3. Add curry powder and chilli powder. Mix for 2 min
4. Add soy sauce, rice vinegar, flour. Mix
5. Add chicken stock. Simmer for 6-8 minutes until thickened.
6. Add honey for 1 minute.
7. Whisk two eggs. Add onion, garlic powder and flour.
   Whisk again.
8. Butterfly chicken.
   Coat in batter. 
   Then place on breadcrumbs.
9. Fry in pan deep in oil for 3 minutes each side.
   Let rest on a wire rack.


Chicken & Crumb - 

2 - Large Chicken Breasts, Butterflied & Halved 
2 - Large Eggs
1 tsp - Onion Powder 
1 tsp - Garlic Powder 
2 Tbsp - Plain (All Purpose) Flour 
Salt & Pepper To Taste
150g - Panko Bread Crumbs 
Oil For Frying 

Curry Sauce - 

1 Tbsp - Vegetable Oil 
2 - Small Brown (Yellow) Onions, Small Diced 
2 - Small Carrots, Peeled & Sliced 
3 - Garlic Cloves, Sliced 
5g - Ginger, Peeled & Thinly Sliced 
2 Tbsp - Curry Powder  (less curry powder)
1/2 tsp - Red Chilli Powder  
2 Tbsp - Plain (All Purpose Flour) 
1 Tbsp - Soy Sauce 
1 tsp - Rice Vinegar 
500ml (2 Cups) - Chicken Stock
1 Tbsp - Honey (less honey)
Seasoning To Taste 

Rice - 

1 Cup (200g) - Jasmine Rice, Washed 
1 1/4 Cups - Cold Water 
Salt To Taste 
'''
